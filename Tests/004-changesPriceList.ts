...
export class ChangesPriceList {
    ...
    public async editPriceByFloor(floorNumber: number, priceModifierType: 'fullPrice' | 'perSquare', 
        changeType: 'increase' | 'decrease' | 'replace', priceUnit: 'percent' | 'currency', value: number): Promise<void>  {
        const selectionFloor = this.page.locator('div[class="floor-number first"]').filter({ has: this.page.getByText(floorNumber.toString()) });
        await selectionFloor.click();
        await this.changePriceButton.click();
        await priceModifierSelect.click();
        const options = this.page.locator('tui-select-option');
        if (priceModifierType === 'fullPrice') {
